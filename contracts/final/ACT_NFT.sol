 ...
 */
contract ActivateEarthNFT is ERC721, Ownable, Pausable, ReentrancyGuard {
    ...
    mapping(uint256 => string) public tokenIdToBaseURI;
    ...
    modifier validNFTInputs(NftTypeInput memory _input) {
        if (_input.price == 0) revert PriceCannotBeZero();
        ...
        if (_input.maxSupply == 0) revert NewMaxSupplyTooLow();
    ...
    function _mintNFT(
        ...
        setTokenURI(tokenId, nftType.baseURI);
     ...
     */
    function setTokenURI(uint256 tokenId, string memory baseURI) internal {
        if (bytes(baseURI).length == 0) revert EmptyBaseURI();
        tokenIdToBaseURI[tokenId] = baseURI;
    }
     ...
     */
    function getGuards() external view onlyOwner returns (address, address) {
